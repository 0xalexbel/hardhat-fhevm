...
import { toBeHexNoPrefix } from "../utils/eth_utils";
...
export function getHandleListType(bits: (keyof typeof FhevmNumBitsType)[]): 160 | 2048 {
  // We limit to 12 items because for now we are using FheUint160List
  if (bits.length > 12) {
    throw new HardhatFhevmError("You can't pack more than 12 values.");
  }

  if (bits.reduce((total, v) => total + v, 0) > 2048) {
    throw new HardhatFhevmError("Too many bits in provided values. Maximum is 2048.");
  }
