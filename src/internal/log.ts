...
// 34  = blue, 35 = purple 31 = red
export function logDimWithYellowPrefix(prefix: string, msg: string, options: LogOptions) {
  const lo = _resolveLogOptions(options);
  if (lo.quiet) {
    return;
  }
  if (lo.nocolor) {
    ...
    _log(`${lo.indent}\x1b[33m${prefix}\x1b[0m\x1b[2m${msg}\x1b[0m`, lo);
  }
}
