...
export class RequestIDDB extends MapID<RequestIDEntry> {
  ...
  public pushRequest(e: EventDecryptionEvent): boolean {
  ...
  public pushResult(e: ResultCallbackEvent): boolean {
    const entry = this.ids.get(e.requestID);
  ...
  public equalRequest(e: EventDecryptionEvent, entry: RequestIDEntry): boolean {
    return (
  ...
  public equalResult(e: ResultCallbackEvent, entry: RequestIDEntry): boolean {
    return (
  ...
  public getPendingRequest(requestID: bigint): EventDecryptionEvent {
  ...
  public getHardhatFhevmDecryptions(requestIDs: bigint[]): HardhatFhevmDecryption[] {
    const decryptions: HardhatFhevmDecryption[] = [];
    for (let i = 0; i < requestIDs.length; ++i) {
      const rID = requestIDs[i];
      if (this.waitingForResult.has(rID)) {
        continue;
      ...
      assert(entry.result);
      assert(entry.request);
      const d: HardhatFhevmDecryption = {
