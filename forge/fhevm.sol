...
library fhevm {
    ...
    function _computeProof(uint64 value, uint8 sizeT, uint8 typeT, bytes32 random) private pure returns (bytes memory result) {
    ...
    // function decrypt64(euint64 value) public view returns (uint64 result){
    //   executor.db(euint64.unwrap(value))
    // }
}