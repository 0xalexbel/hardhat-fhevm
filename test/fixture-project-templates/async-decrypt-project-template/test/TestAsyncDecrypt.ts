...
import { FhevmjsInstances, Signers, createInstances, getSigners, initSigners } from "./test_utils";
import { ethers as EthersT } from "ethers";
import { FhevmjsZKInput } from "../../../types";
import { bigIntToBytes } from "../../../../src/internal/utils/eth_utils";
...
describe("TestAsyncDecrypt", function () {
  ...
  let instances: FhevmjsInstances;
  ...
  before(async function () {
    await initSigners();
    signers = getSigners();
    relayerAddress = hre.fhevm.gateway.relayerWallet.address;
    ...
    await hre.fhevm.helpers.waitNBlocks(1);
  });
  ...
  it("Test1: test async decrypt bool", async function () {
    const caps = await hre.fhevm.getCapabilities();
    if (!caps.supportsAsyncDecryption) {
      console.log(`Network '${hre.network.name}' does not support fhevm async decryption`);
      return;
    }
    ...
    await hre.fhevm.gateway.waitForAllAsyncDecryptions();
  ...
  it("Test11: test async decrypt ebytes256 non-trivial", async function () {
    ...
    if (!caps.supportsEBytes256) {
      // not yet supported
      console.log(`Network '${hre.network.name}' does not support ebytes256`);
      return;
    }
    const inputAlice = instances.alice.createEncryptedInput(contractAddress, signers.alice.address) as FhevmjsZKInput;
  ...
  it("Test12: test async decrypt ebytes256 non-trivial with snapshot [skip-on-coverage]", async function () {
    const caps = await hre.fhevm.getCapabilities();
    if (!caps.rpcMethods.evmRevert || !caps.rpcMethods.evmSnapshot) {
      return;
    }
    if (!caps.supportsAsyncDecryption) {
      console.log(`Network '${hre.network.name}' does not support fhevm async decryption`);
    ...
    const inputAlice2 = instances.alice.createEncryptedInput(contractAddress, signers.alice.address) as FhevmjsZKInput;
    ...
    expect(y2).to.equal(EthersT.toBeHex(424242n, 256));
  });
